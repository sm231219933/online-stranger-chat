...
// Keep existing focused app patches.
...
if(!s.includes('deleteField')){
  s=s.replace('serverTimestamp, setDoc, where } from "firebase/firestore";', 'deleteField, serverTimestamp, setDoc, where } from "firebase/firestore";');
}
...
s=s.replace(
  ...
  'type Profile={uid:string;username:string;country:string;countryCode:string;state:string;stateCode:string;age:number;gender:Gender|"";avatar:string;online?:boolean;lastSeen?:number;logoutAt?:number;emailAccount?:boolean;photoURL?:string};'
);
s=s.replace(
...
// Presence: heartbeat every 30 seconds; after 90 seconds without a heartbeat, show offline.
if(!s.includes('const isActuallyOnline=')){
  s=s.replace('export default function App(){','const isActuallyOnline=(p?:Profile|null)=>!!p&&p.online===true&&typeof p.lastSeen==="number"&&Date.now()-p.lastSeen<90000;\nexport default function App(){');
}
if(!s.includes('[presenceTick')){
  s=s.replace('[error,setError]=useState("")', '[error,setError]=useState(""),[presenceTick,setPresenceTick]=useState(0)');
}
...
// Only show genuinely active users.
s=s.replace(
  ...
  'const filtered=useMemo(()=>{void presenceTick;return profiles.filter(p=>isActuallyOnline(p)&&(!search||p.username.toLowerCase().includes(search.toLowerCase()))&&(!countryFilter||p.country===countryFilter)&&(!stateFilter||p.state===stateFilter)&&(!genderFilter||p.gender===genderFilter))},[profiles,search,countryFilter,stateFilter,genderFilter,presenceTick]);'
);

// Functional state updates prevent controlled inputs from jumping/replacing text while Firebase/presence rerenders.
...
s=s.replace('onChange={e=>setProfile({...profile,country:e.target.value,countryCode:c?.isoCode||"",state:"",stateCode:""})}', 'onChange={e=>{const value=e.target.value,c=countries.find(x=>x.name===value);setProfile(p=>({...p,country:value,countryCode:c?.isoCode||"",state:"",stateCode:""}))}}');
...
// Save/profile/login/account creation keep lastSeen.
...
if(!s.includes('async function updateMessageStatus')){
...
// Remove the right-side HOW TO JOIN panel and the extra sign-up/login prompt from the guest profile flow.
