...
type Profile = { uid: string; username: string; country: string; countryCode: string; state: string; stateCode: string; age: number; gender: "Male" | "Female" | "Neutral"; avatar?: string; online?: boolean; emailAccount?: boolean };
...
const emptyProfile = (uid = ""): Profile => ({ uid, username: "", country: "India", countryCode: "IN", state: "", stateCode: "", age: 18, gender: "Neutral" });
...
function App() {
  ...
  if (screen === "profile" || !profile) {
    ...
    return <main className="app"><section className="card profile-card"><div className="brand"><span>💬</span><div><b>Stranger Chat</b><small>Meet people worldwide</small></div></div><h1>Create your profile</h1><p className="muted">Choose a unique username, country, state, age and gender.</p><form onSubmit={saveProfile}><div className="avatar-row"><div className="avatar large">{p.avatar ? <img src={p.avatar} /> : "👤"}</div><label className="upload">Upload photo<input type="file" accept="image/*" onChange={e => e.target.files?.[0] && uploadImage(e.target.files[0], url => setProfile(x => ({ ...(x || p), avatar: url })))} /></label></div><input required placeholder="Unique username" value={p.username} onChange={e => setProfile({ ...p, username: e.target.value })} /><select value={p.country} onChange={e => changeProfileCountry(e.target.value)}><option value="">Select country</option>{allCountries.map(c => <option key={c.isoCode} value={c.name}>{c.name}</option>)}</select><select required value={p.state} onChange={e => { const s = profileStates.find(x => x.name === e.target.value); setProfile({ ...p, state: e.target.value, stateCode: s?.isoCode || "" }); }}><option value="">Select state / province</option>{profileStates.map(s => <option key={`${s.countryCode}-${s.isoCode}-${s.name}`} value={s.name}>{s.name}</option>)}</select><div className="two"><input required type="number" min="13" max="100" placeholder="Age" value={p.age || ""} onChange={e => setProfile({ ...p, age: Number(e.target.value) })} /><select required value={p.gender} onChange={e => setProfile({ ...p, gender: e.target.value as Profile["gender"] })}><option value="" disabled>Gender</option><option>Male</option><option>Female</option><option>Neutral</option></select></div><button disabled={saving}>{saving ? "Saving…" : "Enter Chat Room →"}</button></form><button className="ghost" onClick={() => setScreen("auth")}>🔐 Sign up / Log in</button>{error && <div className="error">{error}</div>}</section></main>;
  }

  const currentUser = user;
  if (!currentUser) return null;
  return <main className="app room"><aside className="sidebar"><div className="brand"><span>💬</span><div><b>Stranger Chat</b><small>{profiles.length} people</small></div></div><div className="me"><div className="avatar">{profile.avatar ? <img src={profile.avatar} /> : "👤"}</div><div><b>@{profile.username}</b><small>{profile.country} · {profile.gender}</small></div></div><button className="primary" onClick={() => setScreen("profile")}>✏️ Edit profile</button><button className="ghost" onClick={() => setScreen("auth")}>🔐 Account</button></aside><section className="people"><div className="room-head"><div><h2>People</h2><small>Find someone to chat with</small></div></div><div className="search"><span>⌕</span><input placeholder="Search username…" value={search} onChange={e => setSearch(e.target.value)} /></div><div className="filters"><select value={countryFilter} onChange={e => { setCountryFilter(e.target.value); setStateFilter(""); }}><option value="All">Country</option>{allCountries.map(c => <option key={c.isoCode} value={c.name}>{c.name}</option>)}</select><select value={genderFilter} onChange={e => setGenderFilter(e.target.value)}><option value="All">Gender</option>{genders.filter(g => g !== "All").map(g => <option key={g}>{g}</option>)}</select>{countryFilter !== "All" ? <select value={stateFilter} onChange={e => setStateFilter(e.target.value)}><option value="">State</option>{filterStates.map(s => <option key={`${s.isoCode}-${s.name}`} value={s.name}>{s.name}</option>)}</select> : <input placeholder="State" value={stateFilter} onChange={e => setStateFilter(e.target.value)} />}</div><div className="list">{filtered.map(p => <button className="person" key={p.uid} onClick={() => setSelected(p)}><div className="avatar">{p.avatar ? <img src={p.avatar} /> : "👤"}<i /></div><div><b>@{p.username}</b><small>{p.country} · {p.state} · {p.age} · {p.gender}</small></div><span>›</span></button>)}{filtered.length === 0 && <div className="empty">No people found. Try another search or filter.</div>}</div></section><section className="chat">{selected ? <><header><button className="back" onClick={() => setSelected(null)}>‹</button><div className="avatar">{selected.avatar ? <img src={selected.avatar} /> : "👤"}</div><div><b>@{selected.username}</b><small>{selected.country} · {selected.state}</small></div></header><div className="messages">{messages.map((m, i) => <div key={i} className={`bubble ${m.uid === currentUser.uid ? "mine" : ""}`}>{m.image && <img src={m.image} />}{m.text && <div>{m.text}</div>}</div>)}{messages.length === 0 && <div className="empty">Say hello 👋</div>}</div><footer><label className="attach">＋<input type="file" accept="image/*" onChange={e => e.target.files?.[0] && uploadImage(e.target.files[0], sendMessage)} /></label><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === "Enter" && sendMessage()} placeholder="Write a message… 😊" /><button onClick={() => sendMessage()}>➤</button></footer></> : <div className="welcome"><div>💬</div><h2>Choose someone to chat</h2><p>Search the people list and tap a profile to start a private conversation.</p></div>}</section>{error && <div className="toast error">{error}</div>}</main>;
}
